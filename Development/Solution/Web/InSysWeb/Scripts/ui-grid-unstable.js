                                    ...
                                    uiGridCtrl.grid.refresh().then(function () {
                                        if (uiGridCtrl.prevScrollArgs && uiGridCtrl.prevScrollArgs.y && uiGridCtrl.prevScrollArgs.y.percentage) {
                                            uiGridCtrl.fireScrollingEvent({
                                                y: {
                                                    percentage: uiGridCtrl.prevScrollArgs.y.percentage
                                                }
                                            });
                                        }
...
(function () {
    ...
    angular.module('ui.grid').service('uiGridGridMenuService', ['$log', 'i18nService', function ($log, i18nService) {
        /**
         *  @ngdoc service
        ...
        self.rowStyle = function (index) {
            ...
            if (!renderContainer.disableRowOffset) {
                if (index === 0 && self.currentTopRow !== 0) {
                    ...
                    var hiddenRowWidth = ($scope.rowContainer.currentTopRow) * $scope.rowContainer.visibleRowCache[$scope.rowContainer.currentTopRow].height;
...
(function () {
    ...
    angular.module('ui.grid').controller('uiGridController', ['$scope', '$element', '$attrs', '$log', 'gridUtil', '$q', 'uiGridConstants', '$templateCache', 'gridClassFactory', '$timeout', '$parse', '$compile', function ($scope, $elm, $attrs, $log, gridUtil, $q, uiGridConstants, $templateCache, gridClassFactory, $timeout, $parse, $compile) {
        ...
        function dataWatchFunction(n) {
            ...
            if (n) {
                ...
                $q.all(promises).then(function () {
                    self.grid.modifyRows(n).then(function () {
                        // if (self.viewport) {
        ...
        }, self.grid.options.scrollThrottle, {
            trailing: true
        });
     ...
     */
    angular.module('ui.grid').directive('uiGrid', ['$log', '$compile', '$templateCache', 'gridUtil', '$window', function (
    $log, $compile, $templateCache, gridUtil, $window) {
        return {
...
(function () {
    ...
    angular.module('ui.grid').factory('Grid', ['$log', '$q', '$compile', '$parse', 'gridUtil', 'uiGridConstants', 'GridOptions', 'GridColumn', 'GridRow', 'GridApi', 'rowSorter', 'rowSearcher', 'GridRenderContainer', '$timeout', function ($log, $q, $compile, $parse, gridUtil, uiGridConstants, GridOptions, GridColumn, GridRow, GridApi, rowSorter, rowSearcher, GridRenderContainer, $timeout) {
...
(function () {
    ...
    angular.module('ui.grid').factory('GridApi', ['$log', '$q', '$rootScope', 'gridUtil', 'uiGridConstants', 'GridRow', 'uiGridGridMenuService', function ($log, $q, $rootScope, gridUtil, uiGridConstants, GridRow, uiGridGridMenuService) {
        /**
         * @ngdoc function
...
(function () {
    ...
    angular.module('ui.grid').factory('GridColumn', ['gridUtil', 'uiGridConstants', 'i18nService', '$log', function (gridUtil, uiGridConstants, i18nService, $log) {
...
(function () {
    ...
    angular.module('ui.grid').factory('GridOptions', ['gridUtil', function (gridUtil) {
...
(function () {
    ...
    angular.module('ui.grid').factory('GridRenderContainer', ['$log', 'gridUtil', function ($log, gridUtil) {
        ...
        var service = {
            ...
            createGrid: function (options) {
                ...
                if (grid.options.rowTemplate) {
                    ...
                    gridUtil.getTemplate(grid.options.rowTemplate).then(
            ...
            defaultColumnBuilder: function (colDef, col, gridOptions) {
                ...
                col.cellTemplatePromise = gridUtil.getTemplate(colDef.cellTemplate);
                templateGetPromises.push(col.cellTemplatePromise.then(
                ...
                templateGetPromises.push(gridUtil.getTemplate(colDef.headerCellTemplate).then(
...
(function () {
    ...
    function getWidthOrHeight(elem, name, extra) {
        ...
        if (val <= 0 || val == null) {
            ...
            valueIsBorderBox = isBorderBox && (true || val === elem.style[name]); // use 'true' instead of 'support.boxSizingReliable()'
            // Normalize "", auto, and prepare for extra
            val = parseFloat(val) || 0;
        }

        // use the active box-sizing model to add/subtract irrelevant styles
        var ret = (val + augmentWidthOrHeight(
    ...
    module.service('gridUtil', ['$log', '$window', '$document', '$http', '$templateCache', '$timeout', '$injector', '$q', 'uiGridConstants', function ($log, $window, $document, $http, $templateCache, $timeout, $injector, $q, uiGridConstants) {
        var s = {
            ...
            requestAnimationFrame: $window.requestAnimationFrame && $window.requestAnimationFrame.bind($window) || $window.webkitRequestAnimationFrame && $window.webkitRequestAnimationFrame.bind($window) ||
            function (fn) {
        ...
        ['width', 'height'].forEach(function (name) {
            ...
            s['element' + capsName] = function (elem, extra) {
                ...
                if (e) {
                    ...
                    return e.offsetWidth === 0 && rdisplayswap.test(styles.display) ? s.fakeElement(e, cssShow, function (newElm) {
        ...
        s.debounce = function (func, wait, immediate) {
            ...
            function debounce() { /* jshint validthis:true */
                context = this;
                args = arguments;
                var later = function () {
                    timeout = null;
        ...
        s.throttle = function (func, wait, options) {
            ...
            return function () { /* jshint validthis:true */
                context = this;
                args = arguments;
                if (queued === null) {
...
(function () {
     ...
     */
    module.service('uiGridCellNavService', ['$log', 'uiGridConstants', 'uiGridCellNavConstants', '$q', 'uiGridCellNavFactory', function ($log, uiGridConstants, uiGridCellNavConstants, $q, UiGridCellNav) {
        ...
        var service = {
            ...
            getDirection: function (evt) {
                ...
                if (evt.keyCode === uiGridConstants.keymap.DOWN || evt.keyCode === uiGridConstants.keymap.ENTER) {
            ...
            scrollToInternal: function (grid, $scope, gridRow, gridCol) {
                ...
                if (gridRow !== null) {
                    args.y = {
                        percentage: grid.renderContainers.body.visibleRowCache.indexOf(gridRow) / grid.renderContainers.body.visibleRowCache.length
                    };
                }

                if (gridCol !== null) {
                    args.x = {
                        percentage: this.getLeftWidth(grid, gridCol) / this.getLeftWidth(grid, grid.renderContainers.body.visibleColumnCache[grid.renderContainers.body.visibleColumnCache.length - 1])
                    };
                }
    ...
    module.directive('uiGridCell', ['uiGridCellNavService', '$log', 'uiGridCellNavConstants', function (uiGridCellNavService, $log, uiGridCellNavConstants) {
        return {
            priority: -150,
            // run after default uiGridCell directive and ui.grid.edit uiGridCell
            restrict: 'A',
            require: '^uiGrid',
            scope: false,
            link: function ($scope, $elm, $attrs, uiGridCtrl) {
...
(function () {
        ...
        var service = {
            ...
            editColumnBuilder: function (colDef, col, gridOptions) {
                ...
                if (colDef.enableCellEdit) {
                    ...
                    promises.push(gridUtil.getTemplate(colDef.editableCellTemplate).then(
            ...
            isStartEditKey: function (evt) {
                ...
                evt.keyCode === uiGridConstants.keymap.RIGHT || evt.keyCode === uiGridConstants.keymap.TAB ||

                evt.keyCode === uiGridConstants.keymap.UP || (evt.keyCode === uiGridConstants.keymap.ENTER && evt.shiftKey) ||
     ...
     *
     */
    module.directive('uiGridCell', ['$compile', 'uiGridConstants', 'uiGridEditConstants', '$log', '$parse', 'uiGridEditService', function ($compile, uiGridConstants, uiGridEditConstants, $log, $parse, uiGridEditService) {
        return {
            priority: -100,
     ...
     *
     */
    module.directive('uiGridEditor', ['uiGridConstants', 'uiGridEditConstants', function (uiGridConstants, uiGridEditConstants) {
        return {
            scope: true,
            compile: function () {
                return {
...
(function () {
    ...
    module.directive('uiGridExpandableRow', ['uiGridExpandableService', '$timeout', '$log', '$compile', 'uiGridConstants', 'gridUtil', '$interval', function (uiGridExpandableService, $timeout, $log, $compile, uiGridConstants, gridUtil, $interval) {
    ...
    module.directive('uiGridRow', ['$compile', '$log', '$templateCache', function ($compile, $log, $templateCache) {
        return {
            priority: -200,
            scope: false,
            compile: function ($elm, $attrs) {
    ...
    module.directive('uiGridViewport', ['$compile', '$log', '$templateCache', function ($compile, $log, $templateCache) {
        return {
            priority: -200,
            scope: false,
            compile: function ($elm, $attrs) {
    ...
    module.service('uiGridExporterService', ['$log', '$q', 'uiGridExporterConstants', 'gridUtil', '$compile', '$interval', 'i18nService', function ($log, $q, uiGridExporterConstants, gridUtil, $compile, $interval, i18nService) {
        ...
        var service = {
            ...
            renderCsvLink: function (grid, csvContent, $elm) {
                ...
                var linkTemplate = gridUtil.getTemplate(grid.options.exporterLinkTemplate).then(function (contents) {
...
(function () {
    ...
    module.directive('uiGridViewport', ['$compile', '$log', 'uiGridInfiniteScrollService', 'uiGridConstants', function ($compile, $log, uiGridInfiniteScrollService, uiGridConstants) {
        return {
            priority: -200,
            scope: false,
            ...
            pinningColumnBuilder: function (colDef, col, gridOptions) {
                ...
                var pinColumnLeftAction = {
                    ...
                    action: function () {
                        ...
                        col.grid.refresh().then(function () {
                            col.grid.refresh();
                        });
                    }
                };
...
(function () {
     ...
     */
    module.directive('uiGridViewport', ['$compile', 'uiGridConstants', '$log', '$parse', function ($compile, uiGridConstants, $log, $parse) {
        return {
            priority: -200,
            // run after default  directive
            scope: false,
            compile: function ($elm, $attrs) {
                var rowRepeatDiv = angular.element($elm.children().children()[0]);
...
(function () {
     ...
     *  for the grid row
     */
    module.directive('uiGridViewport', ['$compile', 'uiGridConstants', 'uiGridSelectionConstants', '$log', '$parse', 'uiGridSelectionService', function ($compile, uiGridConstants, uiGridSelectionConstants, $log, $parse, uiGridSelectionService) {
        return {
            priority: -200,
            // run after default  directive
            scope: false,
     ...
     */
    module.directive('uiGridCell', ['$compile', 'uiGridConstants', 'uiGridSelectionConstants', '$log', '$parse', 'uiGridSelectionService', function ($compile, uiGridConstants, uiGridSelectionConstants, $log, $parse, uiGridSelectionService) {
        return {
            priority: -200,
            // run after default uiGridCell directive
            restrict: 'A',
...
angular.module('ui.grid').run(['$templateCache', function ($templateCache) {
    ...
    $templateCache.put('ui-grid/ui-grid-footer', "<div class=\"ui-grid-footer-panel\"><div ui-grid-group-panel ng-show=\"grid.options.showGroupPanel\"></div><div class=\"ui-grid-footer ui-grid-footer-viewport\"><div class=\"ui-grid-footer-canvas\"><div ng-repeat=\"col in colContainer.renderedColumns track by col.colDef.name\" ui-grid-footer-cell col=\"col\" render-index=\"$index\" class=\"ui-grid-footer-cell clearfix\" ng-style=\"$index === 0 && colContainer.columnStyle($index)\"></div></div></div></div>");
    ...
    $templateCache.put('ui-grid/ui-grid-header', "<div class=\"ui-grid-header\"><div class=\"ui-grid-top-panel\"><div ui-grid-group-panel ng-show=\"grid.options.showGroupPanel\"></div><div class=\"ui-grid-header-viewport\"><div class=\"ui-grid-header-canvas\"><div class=\"ui-grid-header-cell clearfix\" ng-repeat=\"col in colContainer.renderedColumns track by col.colDef.name\" ui-grid-header-cell col=\"col\" render-index=\"$index\" ng-style=\"$index === 0 && colContainer.columnStyle($index)\"></div></div></div><div ui-grid-menu></div></div></div>");


    $templateCache.put('ui-grid/ui-grid-menu-button', "<div class=\"ui-grid-menu-button\" ng-click=\"toggleMenu()\"><div class=\"ui-grid-icon-container\"><i class=\"ui-grid-icon-menu\">&nbsp;</i></div><div ui-grid-menu menu-items=\"menuItems\"></div></div>");


    $templateCache.put('ui-grid/ui-grid-no-header', "<div class=\"ui-grid-top-panel\"></div>");


    $templateCache.put('ui-grid/ui-grid-row', "<div ng-repeat=\"(colRenderIndex, col) in colContainer.renderedColumns track by col.colDef.name\" class=\"ui-grid-cell\" ng-class=\"{ 'ui-grid-row-header-cell': col.isRowHeader }\" ui-grid-cell></div>");
    ...
    $templateCache.put('ui-grid/cellEditor', "<div><form name=\"inputForm\"><input type=\"{{inputType}}\" ng-class=\"'colt' + col.uid\" ui-grid-editor ng-model=\"MODEL_COL_FIELD\"></form></div>");
    ...
    $templateCache.put('ui-grid/expandableRow', "<div ui-grid-expandable-row ng-if=\"expandableRow.shouldRenderExpand()\" class=\"expandableRow\" style=\"float:left; margin-top: 1px; margin-bottom: 1px\" ng-style=\"{width: (grid.renderContainers.body.getCanvasWidth() - grid.verticalScrollbarWidth) + 'px'\n" + "     , height: grid.options.expandable.expandableRowHeight + 'px'}\"></div>");
    ...
    $templateCache.put('ui-grid/expandableScrollFiller', "<div ng-if=\"expandableRow.shouldRenderFiller()\" style=\"float:left; margin-top: 2px; margin-bottom: 2px\" ng-style=\"{ width: (grid.getViewportWidth()) + 'px',\n" + "              height: grid.options.expandable.expandableRowHeight + 'px', 'margin-left': grid.options.rowHeader.rowHeaderWidth + 'px' }\"><i class=\"ui-grid-icon-spin5 ui-grid-animate-spin\" ng-style=\"{ 'margin-top': ( grid.options.expandable.expandableRowHeight/2 - 5) + 'px',\n" + "            'margin-left' : ((grid.getViewportWidth() - grid.options.rowHeader.rowHeaderWidth)/2 - 5) + 'px' }\"></i></div>");


    $templateCache.put('ui-grid/csvLink', "<span class=\"ui-grid-exporter-csv-link-span\"><a href=\"data:text/csv;charset=UTF-8,CSV_CONTENT\">LINK_LABEL</a></span>");


    $templateCache.put('ui-grid/columnResizer', "<div ui-grid-column-resizer ng-if=\"grid.options.enableColumnResizing\" class=\"ui-grid-column-resizer\" col=\"col\" position=\"right\" render-index=\"renderIndex\"></div>");


    $templateCache.put('ui-grid/selectionHeaderCell', "<div><div class=\"ui-grid-vertical-bar\">&nbsp;</div><div class=\"ui-grid-cell-contents\" col-index=\"renderIndex\"></div></div>");


    $templateCache.put('ui-grid/selectionRowHeader', "<div class=\"ui-grid-row-header-cell ui-grid-disable-selection\"><div class=\"ui-grid-cell-contents\"><ui-grid-selection-row-header-buttons></ui-grid-selection-row-header-buttons></div></div>");
