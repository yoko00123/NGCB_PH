...
(function (window, $) {
        ...
        domUtilityService.BuildStyles = function ($scope, grid, digest) {
            ...
            for (var i = 0; i < cols.length; i++) {
                ...
                if (col.visible !== false) {
                    css += "." + gridId + " .col" + i + " { width: " + col.width + "px; left: " + sumWidth + "px; height: " + (rowHeight + 5) + "px }" +
    ...
    var ngColumn = function (config, $scope, grid, domUtilityService, $templateCache, $utils) {
        ...
        if (config.enablePinning) {
            self.pinnable = false; // $utils.isNullOrUndefined(colDef.pinnable) || colDef.pinnable;
        }
    ...
    var ngFooter = function ($scope, grid) {
        ...
        $scope.maxPages = function () {
            ...
            return $scope.pagingOptions.pageSize == 'All' ? 1 : Math.ceil($scope.maxRows() / $scope.pagingOptions.pageSize);
        };
    ...
    var ngGrid = function ($scope, options, sortService, domUtilityService, $filter, $templateCache, $utils, $timeout, $parse, $http, $q) {
        ...
        self.configureColumnWidths = function () {
            ...
            angular.forEach(self.config.columnDefs, function (colDef, i) {
                ...
                }//console.log( $scope.columns,indexMap[i]);
		...
		$scope.firstAdjustmentLeft = false;
        $scope.adjustScrollLeft = function (scrollLeft) {
            ...
            for (var i = 0; i < x; i++) {
                ...
                if (col.visible !== false) {
					...
					//#EDITED ADD NEW PROPERTY col.left
					if (!$scope.firstAdjustmentLeft){
						addCol(col);	
					}
					if (col.pinned) {
                        //addCol(col);
            ...
            if (dcv && !$scope.firstAdjustmentLeft) {
                $scope.renderedColumns = newCols;
				$scope.firstAdjustmentLeft = true;
            }
        };
        ...
        $scope.toggleSelectAll = function (state, selectOnlyVisible, filter) {

            $scope.selectionProvider.toggleSelectAll(state, false, selectOnlyVisible, filter);
        };
    ...
    var ngSelectionProvider = function (grid, $scope, $parse) {
        ...
        self.toggleSelectAll = function (checkAll, bypass, selectFiltered, filter) {
            ...
            if (bypass || grid.config.beforeSelectionChange(rows, checkAll)) {
                ...
                for (var i = 0; i < rows.length; i++) {
                    ...
                    if (filter !== undefined && filter.length > 0) {
                        try {
                            cond = $parse(filter.replace(/row.entity./g, ''))(rows[i].entity);
                        } catch (e) {
                            cond = false;
                        }
                    }
    ...
    var ngStyleProvider = function ($scope, grid) {
        ...
        $scope.viewportStyle = function () {
            return { "width": grid.rootDim.outerWidth + "px", "height": $scope.viewportDimHeight() + "px" };  //"height": $scope.viewportDimHeight() + "px" //#EDITED "min-height": 0 + "px", "max-height": 300 + "px"
        };
    ...
    angular.module('ngGrid').run(['$templateCache', function ($templateCache) {
        ...
        $templateCache.put('aggregateTemplate.html',
          ...
          "\n" +
          "<span class=\"ngAggregateText\">{{row.label CUSTOM_FILTERS}} ({{row.totalChildren()}})</span>\r" + // {{AggItemsLabel}}
          "\n" +
        ...
        $templateCache.put('footerTemplate.html',
          "<div ng-if=\"showFooter\" class=\"ngFooterPanel\" ng-class=\"{'ui-widget-content': jqueryUITheme, 'ui-corner-bottom': jqueryUITheme}\" ng-style=\"footerStyle()\">\r" +
          "\n" +
          ...
          "</div>\r" +
          "\n" +
          "<div class=\"ngFooterSelectedItems\" ng-if=\"multiSelect\">\r" +
          "\n" +
          ...
          "</div>\r" +
          "\n" +
          "<div class=\"ngPagerContainer\" style=\"float: right; margin-top: 10px;\" ng-if=\"enablePaging\" ng-class=\"{'ngNoMultiSelect': !multiSelect}\">\r" +
          "\n" +
          ...
          "\n" +
          "<option sly-repeat=\"size in pagingOptions.pageSizes\">{{size}}</option>\r" +
          "\n" +
          "</select>\r" +
          "\n" +
          "</div>\r" +
        ...
        $templateCache.put('gridTemplate.html',
          ...
          "\n" + 
          "<div ng-style=\"rowStyle(row)\" ng-repeat=\"row in renderedRows\"  ng-click=\"row.toggleSelected($event)\" ng-class=\"row.alternatingRowClass()\" ng-row></div>\r" + //REMOVED 
          "\n" +
          "</div>\r" +
          "\n" +
          "</div>\r" +
        ...
        $templateCache.put('headerCellTemplate.html',
          "<div class=\"ngHeaderSortColumn {{col.headerClass}}\" >\r" + //REMOVED ng-style=\"{'cursor': col.cursor}\" ng-class=\"{ 'ngSorted': !col.noSortVisible() }\"
          "\n" +
          "<div ng-click=\"col.sort($event)\" ng-class=\"'colt' + col.index\" class=\"ngHeaderText\">{{col.displayName}}</div>\r" +
          "\n" +
          "<div class=\"ngSortButtonDown\" ng-click=\"col.sort($event)\" ng-if=\"col.showSortButtonDown()\"></div>\r" +
          "\n" +
          "<div class=\"ngSortButtonUp\" ng-click=\"col.sort($event)\" ng-if=\"col.showSortButtonUp()\"></div>\r" +
          "\n" +
          "<div class=\"ngSortPriority\">{{col.sortPriority}}</div>\r" +
          "\n" +
          "<div ng-class=\"{ ngPinnedIcon: col.pinned, ngUnPinnedIcon: !col.pinned }\" ng-click=\"togglePin(col)\" ng-if=\"col.pinnable\"></div>\r" +
          "\n" +
          "</div>\r" +
          "\n" +
          "<div ng-if=\"col.resizable\" class=\"ngHeaderGrip\" ng-click=\"col.gripClick($event)\" ng-mousedown=\"col.gripOnMouseDown($event)\"></div>\r" +
          "\n"
        );
        $templateCache.put('headerRowTemplate.html', //#EDITED left : col.left '' adjustScrollLeft
          "<div ng-style=\"{ height: col.headerRowHeight, left : col.left }\" sly-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngHeaderCell\">\r" +
          "\n" +
          "\t<div class=\"ngVerticalBar ngVerticalBarVisible\" ng-style=\"{height: col.headerRowHeight}\">&nbsp;</div>\r" + //REMOVED ng-class=\"{ ngVerticalBarVisible: !$last }\"
          "\n" +
        ...
        $templateCache.put('menuTemplate.html',
          "<div ng-if=\"showColumnMenu || showFilter\"  class=\"ngHeaderButton\" ng-click=\"toggleShowMenu()\">\r" +
          "\n" +
          "<div class=\"ngHeaderButtonArrow\"></div>\r" +
          "\n" +
          "</div>\r" +
          "\n" +
          "<div ng-if=\"showMenu\" class=\"ngColMenu\">\r" +
          "\n" +
          "<div ng-if=\"showFilter\">\r" +
          "\n" +
          "<input placeholder=\"{{i18n.ngSearchPlaceHolder}}\" type=\"text\" ng-model=\"filterText\"/>\r" +
          "\n" +
          "</div>\r" +
          "\n" +
          "<div ng-if=\"showColumnMenu\">\r" +
          "\n" +
        ...
        $templateCache.put('rowTemplate.html', //#EDITED left : col.left '' adjustScrollLeft
          "<div ng-style=\"{ 'cursor': row.cursor, 'left' : col.left }\"  ng-repeat=\"col in renderedColumns\" ng-class=\"col.colIndex()\" class=\"ngCell {{col.cellClass}}\">\r" +
          "\n" +
          "\t<div class=\"ngVerticalBar ngVerticalBarVisible\"   >&nbsp;</div>\r" + //removed ng-class=\"{ ngVerticalBarVisible: !$last }\" ng-style=\"{height: rowHeight}\" 
          "\n" +
          "\t<div ng-cell></div>\r" +
          "\n" +
          "</div>"
