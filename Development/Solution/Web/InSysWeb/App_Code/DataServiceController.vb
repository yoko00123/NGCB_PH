...
<DeflateCompression>
<CheckCSRFHeader>
    Public Function DropNotifications(ByVal obj As Newtonsoft.Json.Linq.JObject) As HttpResponseMessage
        If IsNull(HttpContext.Current.Session("ID_User"), 0) = 0 Then Return New HttpResponseMessage(System.Net.HttpStatusCode.Unauthorized)
       Dim ID_User As Integer = HttpContext.Current.Session("ID_User"),
            IsView As String = 0,
            message As String = String.Empty
     ...
     Try
            Using sqlCon As New SqlConnection(ConnectionString)
                sqlCon.Open()
                Using sqlCmd As New SqlCommand("Delete from tWebUserNotifications WHERE ID_User = @ID_User and IsView = 1", sqlCon)
                    Try
...
'clear Notification'
   ...
   <HttpPost>
    <DeflateCompression>
    <CheckCSRFHeader>
    Public Function ClearNotification(ByVal obj As Newtonsoft.Json.Linq.JObject) As HttpResponseMessage
        If IsNull(HttpContext.Current.Session("ID_User"), 0) = 0 Then Return New HttpResponseMessage(System.Net.HttpStatusCode.Unauthorized)
        Dim ID_User As Integer = HttpContext.Current.Session("ID_User"),
            IsView As String = 0,
            message As String = String.Empty
        ...
        Try
            Using sqlCon As New SqlConnection(ConnectionString)
                sqlCon.Open()
                Using sqlCmd As New SqlCommand("Update dbo.tWebUserNotifications SET IsView = 1 where ID_User = @ID", sqlCon)
                    Try
