...
function fcheckchildren(prowid, pshow) {
    ...
    var cb = document.getElementById('chk' + prowid);
    if (tr.getAttribute('data-id')) {
        faddremovechecked(tr.getAttribute('data-id'), cb.checked);
    }
    ...
    if (lvl) {
        for (x = parseInt(prowid) + 1; x < trs.length; x++) {
            ...
            if (nx) {
                ...
                if (nx.getAttribute('data-id')) {
                    var cb = document.getElementById('chk' + nx.getAttribute('data-id'));
                    //                    alert(cb.id);
                    //                                    if (nx.getAttribute('data-id')) {
                   // alert(pshow);
                    faddremovechecked(nx.getAttribute('data-id'), pshow);
                    //                                    }
...
function faddremovechecked(pid, padd) {
    if (1 == 1) {
        try {
            window.external.AddRemoveChecked(pid, padd);
        }
        catch (err) {
            alert(err);
        }
