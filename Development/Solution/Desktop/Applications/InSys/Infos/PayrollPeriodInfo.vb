...
#Region "LoadInfo"
    Public Overrides Sub LoadInfo(ByVal pID As Integer)
        ...
        'End If
        Dim cs As String
        If nDB.GetCompanyID IsNot DBNull.Value Then
            cs = nDB.GetCompanyID
        Else
            cs = "NULL"
        End If
        Dim d As String = "SELECT IsNull(dbo.fGetCompanySetting('EnableAddEmployeeButton'," & cs & "),0)"
        ...
        Try
            ...
            Select Case a.Result
                ...
                Case MessageBoxForm.MgsResult.Resume
                    Dim sqlTemp As String = "SELECT * FROM dbo.fPayslip(" & pIDTemp & "," & nDB.GetUserID & ")" & _
...
Retry1:             vTime = Now
                    ...
                    Try
                        ...
                        'Next
                        reportTemp.ExportToDisk(CrystalDecisions.Shared.ExportFormatType.PortableDocFormat, My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\Payslip.pdf")
                        Application.DoEvents()
                    Catch ex As Exception
                    ...
                    mMsgTemp.Attachments.Add(New Attachment(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\Payslip.pdf"))
...
Retry:
                    ...
                    Try
                        ...
                        If IO.File.Exists(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\Payslip.pdf") Then
                            mMsgTemp.Dispose()
                            mMsgTemp = Nothing
                            IO.File.Delete(My.Computer.FileSystem.SpecialDirectories.MyDocuments & "\Payslip.pdf")
                        End If
                    Catch ex As Exception
